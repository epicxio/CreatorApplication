...
export const useChatWidget = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      chatService.getAvailableChatUsers(user.id).then(setAvailableUsers);
    }
  }, [user]);
  ...
  const scheduleChat = async (scheduledTime: Date, duration: number = 30) => {
    ...
    setIsLoading(true);
    try {
      await chatService.scheduleChat(user.id, scheduledTime, duration);
      return true;
    } catch (error) {
  ...
  const canChatWithUser = async (targetUserId: string) => {
    ...
    try {
      return await chatService.canChatWithUser(user.id, targetUserId);
    } catch (error) {
